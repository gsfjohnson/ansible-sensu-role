...
#    du.root, du.root.boot, and du.root.media.sda1
...
#    du.root, du.boot, and du.media_sda1
#
  ...
  def run
    ...
    `df -PB#{config[:block_size]} #{config[:local] ? '-l' : ''} #{type_option}`.split("\n").drop(1).each do |line| # rubocop:disable Style/Next
      ...
      unless %r{/sys|/dev|/run}.match(mnt)
        ...
        mnt = mnt.gsub '/', delim
        output [config[:scheme], 'du', mnt, 'used'].join('.'), used.gsub(config[:block_size], '')
        output [config[:scheme], 'du', mnt, 'avail'].join('.'), avail.gsub(config[:block_size], '')
        output [config[:scheme], 'du', mnt, 'used_pc'].join('.'), used_p.gsub('%', '')
      end
    end
    ok
  end
